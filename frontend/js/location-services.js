...
// Frontend helper for distance calculations and map rendering
...
class LocationServices {
  ...
  async initialize() {
    try {
      ...
      await this.ensureGoogleMapsLoaded(this.config.apiKey);
      this.initializeServices();
      return true;
    } catch (error) {
      console.error('[LocationServices] Failed to initialize:', error);
      throw error;
    }
  }
  ...
  async fetchConfig() {
    try {
      ...
      if (!payload?.success || !payload.data) {
        throw new Error('Invalid Google Maps config payload');
      }

      return payload.data;
    } catch (error) {
      console.error('[LocationServices] Config fetch failed:', error);
      throw error;
    }
  }

  async ensureGoogleMapsLoaded(apiKey) {
  ...
  async calculateDistances(apartmentAddress, city) {
    ...
    for (const landmark of landmarks) {
      try {
        ...
        console.warn(`[LocationServices] Failed to calculate distance to ${landmark.name}:`, error);
      }
    }
  ...
  createDistanceWidget(apartmentAddress, city) {
    ...
    this.calculateDistances(apartmentAddress, city)
      .then(distances => {
        this.displayDistances(distances, widget);
      })
      .catch(error => {
        console.error('[LocationServices] Error calculating distances:', error);
  ...
  initializeMap(containerId, apartmentLocation) {
    if (!apartmentLocation && this.config?.defaultCenter) {
      apartmentLocation = this.config.defaultCenter;
    }

    const mapContainer = document.getElementById(containerId);
    if (!mapContainer || !apartmentLocation) {
      return;
    }
