# 🎯 SICHRPLACE API ENDPOINTS - GERMAN SCHEMA COMPATIBILITY COMPLETE
...
## 🚨 Diagnostics Snapshot (2025-09-30)
...
| `js/backend/api/secure-videos.js` | Upload route fails under test with “multer is not a function,” signalling the dependency isn’t loaded or is mocked away. | Confirm `multer` dependency, avoid Jest overrides, and optionally lazy-load the module. |
...
| `js/backend/api/viewing-request.js` | `router.post('/viewing-request', …)` plus server mount at `/api/viewing-request` yields `/api/viewing-request/viewing-request`; `EmailService` may still be unready because its constructor is async. | Change to `router.post('/', …)` and add a readiness check for `EmailService`. |
...
| `js/backend/services/AnalyticsDashboardService.js` | Calls RPC/tables (`get_search_stats`, `popular_searches`, `search_locations`) that aren’t provisioned in dev, leading to 500s. | Add existence checks and fall back to mock data or empty stats when tables/procs are absent. |
| `js/backend/services/ApartmentService.js` | Filters rely on columns like `kaltmiete_plus_nebenkosten` and status `verfuegbar` that don’t exist in current schema. | Update queries to use actual columns/RPCs or compute warm rent in code before filtering. |
| `js/backend/services/DatabasePerformanceService.js` | Uses Redis cache helpers (`cacheService.setWithExpiry`, `.getCacheStats`) that aren’t implemented and references columns (`requester_id`, `landlord_id`) removed in German schema. | Align with available Redis API, update column names, and guard against missing Supabase views. |
...
| `netlify/functions/consent-management.mjs` | Uses the service-role key for consent CRUD, refers to helpers like `updateUserPreferencesFromConsent` that aren’t defined, and leaks full audit trails without guardrails. | Move consent workflows to a secured backend service, rely on anon key + RLS, and implement the missing helpers before re-enabling. |
